export async function run(hazel, core, hold, socket, data) {
    if (data.find == 'all') {
        let result = hold.noticeList.map(notice => notice + `, 编号为: ${hold.noticeList.indexOf(notice) + 1}`);
        core.reply({
            cmd: 'info',
            code: 'NOTICE_LIST',
        ...
        if (data.type == 'text') {
        let result = hold.noticeList.filter(notice => notice.includes(data.find) + `, 编号为: ${hold.noticeList.indexOf(notice) + 1}`);
        if (result.length == 0) {
            core.reply({
                cmd: 'info',
